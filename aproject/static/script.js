...
const renderCalendar = () => {
    ...
    for (let i = firstDayofMonth; i > 0; i--) { // creating li of previous month last days
        liTag += `<li class="inactive" onclick="redirectToDay('${currMonth}', '${lastDateofLastMonth - i + 1}', '${currYear}')" > ${lastDateofLastMonth - i + 1} </li>`;
    }
    ...
    for (let i = 1; i <= lastDateofMonth; i++) { // creating li of all days of current month
        ...
        liTag += `<li class="${isToday}" onclick="redirectToDay('${currMonth + 1}', '${i}', '${currYear}')" > ${i} </li>`;
    }

    for (let i = lastDayofMonth; i < 6; i++) { // creating li of next month first days
        liTag += `<li class="inactive" onclick="redirectToDay('${currMonth + 2}', '${i - lastDayofMonth + 1}', '${currYear}')" > ${i - lastDayofMonth + 1} </li>`
    }
...
function redirectToDay(month, day, year) {
